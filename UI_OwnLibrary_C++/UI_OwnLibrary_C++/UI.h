...
void mainMenu(string title) {
	...
	for (int counter = 2; counter <= 15; counter++) {  // 15
		gotoXY(3, counter);
		textColor(368);
		cout << " ";
	}
	...
	for (int counter = 4; counter <= 15; counter++) {  //13
		gotoXY(23, counter);
		textColor(368);
		cout << " ";
	}
	...
	for (int infiniteCounter = 0;;) {	// for infinite loop
		...
		if (key == KEY(KEY_UP) && (counter >= 2 && counter <= 3 || counter >= 3 && counter <= 4 || counter >= 4 && counter <= 5)) {	// for up key
			counter--;
		}

		//( 1,2,    ,2,3)    (,2,3   ,3,4)	(3,4   4,5)     
		if (key == KEY(KEY_DOWN) && (counter >= 1 && counter <= 2 || counter >= 2 && counter <= 3 || counter >= 3 && counter <= 4)) {	// for down key
			counter++;
		}

		if (key == '\r') {	// for Enter key
			if (counter == 1) {
				system("cls");
